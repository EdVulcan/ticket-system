...
- 悬浮助手使用独立的 `AgentTask` 保存多轮上下文和状态。除批量票规外，`ticket_product_create` 任务会收集票种名称、景区、售价、结算价和至少一个检票点；信息不完整时只返回缺失问题，不创建 Product。信息完整后的预览固定显示 `status=offline`、`is_distributable=false`，且不创建 ProductOffer、SellerListing 或渠道映射；确认后才在任务事务内调用现有产品创建服务。任务按租户和操作者隔离、带幂等键和过期时间，刷新页面可继续未完成任务。
