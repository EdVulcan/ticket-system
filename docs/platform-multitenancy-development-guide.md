...
- Agent 规划器的模型输出必须经过独立服务端策略层：只接受受支持的操作类型，拒绝混合计划、未知字段和模型伪造的数据库编号；批量操作方向需与用户输入没有明显冲突，全量票种范围必须由用户明确表达。租户目录名称和任务上下文按纯数据发送给 provider，产品创建预览只返回业务名称并完整展示所有将写入字段，不暴露内部租户、景区、规则或检票点 ID。
