...
> **实现复核（2026-08-01）**：数据库迁移当前版本为 51。平台治理工作台、供应商 Listing 状态同步、游客快照归属、POS 收银闭环、按原支付来源分摊的混合退款、供应履约详情、上下游结算工作台和按票售后已进入主线。改期和换票可按游客票码拆分；换票价差通过独立用途的支付/退款事实结清，不冒充普通下单或退票；结算价例外必须由主管记录原因。这些改动改善了运营可见性，但不改变真实支付、渠道、硬件、双方对账制度和现场验收仍是生产准入门槛的结论。当前实施顺序见[当前开发推进路线](./current-development-roadmap-2026-08-01.md)。
...
- 增加 `ProductRevision`、整数分 `LedgerEntry`、原订单现金/授信构成、`SettlementStatement/Line`，结算按供应履约快照生成，不从当前商品反推；迁移版本已推进到 51，并有迁移幂等回归测试。
...
当前状态：P0-1 至 P0-6 已通过本地自动化验收，数据库迁移已推进到版本 51 并通过重复执行幂等测试。`ScenicArea`、租户状态/能力、审计日志、`ProductRevision`、`ProductOffer/SellerListing`、`FulfillmentOrder/TicketEntitlement`、整数分账本、结算单、追加式结算调整和售后价差事实均已进入当前模型。阶段 0 仍未达到生产准入：旧数据完整迁移、数据库复合约束、真实支付商户证书、双方对账验收、前端 E2E 及现场硬件联调尚未完成。

### 阶段 1：完成景区直营生产闭环

状态：后端已具备库存预占/释放、票权益快照、按原支付来源退款、支付补偿任务、POS 班次、打印任务、设备告警和按票改期/换票；窗口端已开始由 Electron 迁移到 Wails v2，并按售票员高频工作流重构界面。数字退款供应商适配、报表口径确认、真实设备协议和现场全链路验收仍未完成。
