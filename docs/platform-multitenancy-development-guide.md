...
> **门票打印模板补充（2026-08-19）**：PostgreSQL schema 已升级至 103。景区供应商可在管理端按景区、票种或产品 revision 维护受限的结构化打印区块，服务端预览后保存草稿并发布不可变版本。出票/补打任务在调用硬件前生成并持久化 `PrintDocument` 内容快照、模板版本和哈希；模板后续发布不会回写既有任务。混合订单若命中不同模板，POS 按票逐张排队；旧的整单快照调用 fail-closed。模板不接受任意 HTML、JavaScript、SQL 或渠道/结算敏感字段，真实打印机仍需现场协议和设备验收，未配置硬件时不得假成功。
...
> **当前 schema 复核（2026-08-19）**：当前总 schema 为 103。除前述酒店和 Agent 事实外，打印模板/版本/打印任务快照已纳入迁移、租户景区归属触发器和自动化测试；历史段落中的旧 schema 数字仅描述当时状态。
