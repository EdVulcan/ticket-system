...
- AI 连接由平台管理员统一配置（当前支持 DeepSeek 和 OpenAI 兼容接口），API Key 使用平台加密密钥保存且只写不读；租户不配置自己的 Key。每个租户按月记录请求数和 Token 数，达到平台为该租户分配的额度时拒绝请求；未来可在此账本上增加套餐、计费和租户级额度，不改变业务执行边界。
