...
> **当前审查状态（2026-08-12）**：代码级 P0/P1 已完成一轮闭环并进入自动化，PostgreSQL schema 版本为 83；携程供应商订单沙箱和价格库存推送已完成当前可执行测试，小红书商家自开发小程序已完成登录、商品发布配置、订单创建、担保支付查询收敛和酒景套餐售后预约代码，真实支付、退款、核销和结算仍待沙箱及生产验收。团队域已补齐独立合作关系、计划与档案维护、名单/票权归类、并发分批入园、退款投影和双方结算状态机，但这只代表代码与自动化边界收敛，不能替代生产验收。当前生产阻断项仍为真实支付、真实硬件、真实渠道生产验收、部署恢复演练及现场容量/长稳验收。旧库导入不是新系统发布门槛，仅在以后明确提出时作为独立项目处理。当前执行状态以[当前开发推进路线](./current-development-roadmap-2026-08-01.md)和[现场联调准备清单](./field-integration-readiness-checklist.md)为准。
...
- schema 83 为酒景套餐增加两种互斥销售模式：`at_purchase` 在购买时选入住日期并同步占用票日期库存和连续房晚；`after_purchase` 在支付时只生成逐份预约权益和未激活票权，后续由游客逐份选择入住日期，预约成功后才占用对应日期的门票与房晚并生成可核销票码。预约、取消预约、允许次数内改约、退款和过期必须在同一事务同步处理票权、日期库存、房量及预约权益，不能只改其中一侧。
...
- 小红书商城目录按实际商品区分 `ticket` 与 `scenic_hotel_package`。购买时定日期的套餐在确认订单时采集入住日期和联系人；售后预约套餐必须发布为小红书预售券，购买时不采集日期，支付后在订单详情逐份预约、取消或改约，预约成功前不展示可核销票码。纯景区租户不会看到酒店分类，纯酒店租户在尚无独立住宿商品前也不展示虚假门票或空酒店商城。
...
> **实现复核（2026-08-12）**：数据库迁移当前版本为 83。平台治理工作台已将市场身份与供应业态分离，当前供应业态仅开放景区票务和酒店住宿，现有有效供应商兼容回填为景区票务。酒店资料、房型、价格计划、每日房量和同一供应商固定酒景套餐已进入当前实现；酒景套餐已具备轻量住宿名单、入住/离店/未到店履约、购买后逐份预约、改期时票与客房库存同步迁移、名单导出和经营汇总，但单酒店在线预订、独立酒店订单、夜审、独立酒店结算及跨供应商酒景套餐仍未建设。供应商 Listing 状态同步、跨供应商纯门票组合产品、游客快照归属、POS 收银闭环、按原支付来源分摊的混合退款、供应履约详情、销售订单多供应商责任视图、上下游结算工作台、按票售后、旅行社团队履约闭环以及营业/核销四张正式报表已进入当前实现。团队合作不会从普通分销关系推断，合同产品价也不会创建或覆盖普通 `ProductOffer`；团队计划、业务员/导游/车辆档案、名单票权归类、分批入园、退款投影和结算付款状态均保留审计。小红书游客身份、商品配置、订单和券码关联按租户及渠道账号隔离，官方 `session_key`、支付令牌和券码均加密保存且不下发无关客户端。这些改动改善了业务完整性和运营可见性，但不改变真实支付、渠道、硬件、双方现场制度和生产验收仍是准入门槛的结论。当前实施顺序见[当前开发推进路线](./current-development-roadmap-2026-08-01.md)。
...
当前状态：P0-1 至 P0-6 已通过本地自动化验收，数据库 schema 版本为 83，PostgreSQL 初始化和重复执行幂等测试已通过。`ScenicArea`、租户状态/能力、供应商业态、审计日志、`ProductRevision`、`ProductOffer/SellerListing`、`BundleProduct/BundleVersion/BundleComponent`、`FulfillmentOrder/TicketEntitlement`、整数分账本、结算单、追加式结算调整、售后价差事实、核销冲销审计、团队重复归属保护、小红书渠道游客、商品、订单、券码及酒景预约权益均已进入当前模型。阶段 0 仍未达到生产准入：真实支付商户证书、双方对账验收及现场硬件联调尚未完成；旧库导入仅在以后明确提出时另行验收。
