...
> **实现复核（2026-08-01）**：数据库迁移当前版本为 50。平台治理工作台、供应商 Listing 状态同步、游客快照归属、POS 收银闭环、按原支付来源分摊的混合退款、供应履约详情和上下游结算工作台已进入主线。混合退款只在全部渠道退款确认后统一结转票权、库存和结算事实；结算争议只追加调整并重新确认，不覆盖履约行。这些改动改善了运营可见性，但不改变真实支付、渠道、硬件、双方对账制度和现场验收仍是生产准入门槛的结论。当前实施顺序见[当前开发推进路线](./current-development-roadmap-2026-08-01.md)。
...
当前状态：P0-1 至 P0-6 已通过本地自动化验收，数据库迁移已推进到版本 50 并通过重复执行幂等测试。`ScenicArea`、租户状态/能力、审计日志、`ProductRevision`、`ProductOffer/SellerListing`、`FulfillmentOrder/TicketEntitlement`、整数分账本、结算单和追加式结算调整均已进入当前模型。阶段 0 仍未达到生产准入：旧数据完整迁移、数据库复合约束、真实支付商户证书、双方对账验收、前端 E2E 及现场硬件联调尚未完成。

### 阶段 1：完成景区直营生产闭环

状态：后端已具备库存预占/释放、票权益快照、现金退款、支付补偿任务、POS 班次、打印任务和设备告警基础；窗口端已开始由 Electron 迁移到 Wails v2，并按售票员高频工作流重构界面。改期/作废、数字退款供应商适配、报表口径确认、真实设备协议和现场全链路验收仍未完成。
...
状态：报价、销售映射、供应履约单、票权益、资金账本和结算单基础已落地，并有跨景区核销和篡改防护测试；供应商履约详情已覆盖游客、票、核销、售后和退款冲减，结算工作台可生成、双方确认、争议调整、重确认和登记付款，供应商也可查看分销账户与流水。多供应商页面级责任验收、结算导出和双方现场对账仍未完成。
