...
> **实现复核（2026-08-01）**：数据库迁移当前版本为 47。平台治理工作台、供应商 Listing 状态同步、游客快照归属、POS 班次汇总、现金实付/找零和追加式班次更正已进入主线；这些改动改善了运营可见性，但不改变真实支付、渠道、硬件、财务制度和现场验收仍是生产准入门槛的结论。当前实施顺序见[当前开发推进路线](./current-development-roadmap-2026-08-01.md)。
...
当前状态：P0-1 至 P0-6 已通过本地自动化验收，数据库迁移已推进到版本 47 并通过重复执行幂等测试。`ScenicArea`、租户状态/能力、审计日志、`ProductRevision`、`ProductOffer/SellerListing`、`FulfillmentOrder/TicketEntitlement`、整数分账本和结算单均已进入当前模型。阶段 0 仍未达到生产准入：旧数据完整迁移、数据库复合约束、真实支付商户证书、财务对账、前端 E2E 及现场硬件联调尚未完成。
