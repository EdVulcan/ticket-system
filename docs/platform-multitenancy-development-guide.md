...
> **酒店 Agent 边界（2026-08-18）**：AI 助手已覆盖酒店模块的受限能力。酒店供应商可让 Agent 查询酒店/房型/价格计划/独立酒店产品、日期房量、两套价格日历、酒景套餐住宿预订/预约权益和经营汇总；房量、房型价格计划入住日覆盖价、日历房产品销售价日历以及单笔酒景住宿预订的入住/离店/未到店登记，均须先生成逐日或逐预订预览并由用户确认，确认时复查租户、酒店业态、权限、版本和快照。酒店创建/删除、基础配置、上下架、分销/渠道发布、预约创建/取消/改期/退款、支付/结算、外部渠道/PMS/Webhook 和独立酒店履约状态写入仍保持 fail-closed。酒店 Agent 不是 PMS。
...
> **当前审查状态（2026-08-19 修复补充）**：代码级 P0/P1 已完成一轮闭环并进入自动化，PostgreSQL schema 版本为 101；酒店模块已补齐日期范围/星期批量房量配置和价格计划入住日期覆盖，日期价未覆盖时回退基础价，历史售出事实仍使用原快照；携程供应商订单沙箱和价格库存推送已完成当前可执行测试，小红书商家自开发小程序已完成登录、商品发布配置、订单创建、担保支付查询收敛，以及酒景套餐预约、撤销和预约状态通知的可重启任务。小红书预约 `/component` 接口已与普通商家 `/mp` 鉴权分离，但服务商 `appid` 和已授权小程序组件凭据尚未配置，缺少时预约会 fail-closed；这不等于生产预约可用。小红书担保支付主动退款、平台售后/退款事件消费、凭证核销和结算仍待协议实现及沙箱、生产验收，未完成前后台退款入口必须明确失败，不能只改本地票权或模拟成功。平台 AI 助手已支持统一平台模型配置、租户月度请求/Token 额度，以及批量票规和新票种创建的多轮任务预览/确认；第一阶段已建立版本化系统知识契约、能力注册表和任务知识冻结，第二阶段已开放订单、票种库存、销售汇总和核销汇总的只读 QueryResult，第三阶段已开放当前租户自有票种的基础信息修改（分销副本除外），阶段五补充租户业务别名、Provider 瞬态失败保护、无密钥 provider-independent 评测回归和 2-5 步低风险复合预览，阶段七接入酒店目录/房量/两套价格日历/酒景套餐住宿履约查询与低风险预览确认。2026-08-19 又统一了系统知识契约、旧 JSON 协议和酒店上下文；酒店查询要求原生 `tool_v1`，旧协议仅兼容受限酒店预览写入，酒店结构、预约、退款、渠道、PMS、Webhook、支付结算等仍 fail-closed。Agent 任务幂等键按租户/操作者隔离、显式跨租户限定在任务创建前拒绝、工具 allowlist 和单工具强制选择仍由服务端执行；完整后端测试与管理端构建通过。DeepSeek 新任务默认使用原生工具调用，旧默认配置会在 schema 94 迁移为 `auto`，普通文本不会被当作可执行计划，默认请求超时 120 秒，仍由服务端字段、租户和业务规则校验，模型异常时 fail-closed。复合预览仅编排既有低风险步骤，确认后顺序执行且不承诺跨域原子性；分销、渠道及所有高风险写入仍未开放给 Agent。团队域已补齐独立合作关系、计划与档案维护、名单/票权归类、并发分批入园、退款投影和双方结算状态机，但这只代表代码与自动化边界收敛，不能替代生产验收。当前生产阻断项仍为真实支付、真实硬件、真实渠道生产验收、部署恢复演练及现场容量/长稳验收。旧库导入不是新系统发布门槛，仅在以后明确提出时作为独立项目处理。当前执行状态以[当前开发推进路线](./current-development-roadmap-2026-08-01.md)和[现场联调准备清单](./field-integration-readiness-checklist.md)为准。
