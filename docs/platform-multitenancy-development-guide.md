...
> **当前审查状态（2026-08-02）**：代码级 P0/P1 已完成一轮闭环并进入自动化，迁移版本为 58；当前生产阻断项已收敛为真实支付、真实硬件、真实渠道协议、真实旧库样本、部署恢复演练及现场容量/长稳验收。当前执行状态以[当前开发推进路线](./current-development-roadmap-2026-08-01.md)和[现场联调准备清单](./field-integration-readiness-checklist.md)为准。
...
当前状态：P0-1 至 P0-6 已通过本地自动化验收，数据库迁移已推进到版本 58 并通过重复执行幂等测试。`ScenicArea`、租户状态/能力、审计日志、`ProductRevision`、`ProductOffer/SellerListing`、`BundleProduct/BundleVersion/BundleComponent`、`FulfillmentOrder/TicketEntitlement`、整数分账本、结算单、追加式结算调整、售后价差事实和核销冲销审计均已进入当前模型。阶段 0 仍未达到生产准入：真实旧库完整迁移、真实支付商户证书、双方对账验收及现场硬件联调尚未完成。
