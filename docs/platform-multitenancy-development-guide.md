...
> **阶段五修订（2026-08-16，阶段六补充 2026-08-17）**：PostgreSQL schema 已升级至 99；Agent 增加租户业务别名、Provider 瞬态失败保护、无密钥 provider-independent 评测回归和 2-5 步低风险复合预览。别名只绑定当前租户现有景区、检票点或自有票种，解析目标变化时 fail-closed；Provider 只对网络、429 和 5xx 做一次受控重试，连续失败短暂熔断，401/其他 4xx 不重试。复合父任务只编排既有预览/确认服务，步骤之间不是跨域原子事务，未新增高风险执行工具。酒店价格计划现在支持按入住日期维护可选价格覆盖，未覆盖日期沿用基础价，已售套餐和历史预约快照不被回写。

> **阶段五当前实现补充（2026-08-16，阶段六补充 2026-08-17）**：上方历史审查段落中的 schema 97 和“多步骤组合预览未设计”仅保留为历史记录；当前实现以 schema 99 为准，复合父任务已开放 2-5 个低风险预览步骤，确认后顺序执行且不承诺跨域原子性。酒店模块仍是轻量住宿履约协调，不扩展为 PMS；房量可按日期范围和星期批量套用，价格计划可维护入住日期覆盖。
...
> **当前审查状态（2026-08-17）**：代码级 P0/P1 已完成一轮闭环并进入自动化，PostgreSQL schema 版本为 99；酒店模块已补齐日期范围/星期批量房量配置和价格计划入住日期覆盖，日期价未覆盖时回退基础价，历史售出事实仍使用原快照；携程供应商订单沙箱和价格库存推送已完成当前可执行测试，小红书商家自开发小程序已完成登录、商品发布配置、订单创建、担保支付查询收敛，以及酒景套餐预约、撤销和预约状态通知的可重启任务。小红书预约 `/component` 接口已与普通商家 `/mp` 鉴权分离，但服务商 `appid` 和已授权小程序组件凭据尚未配置，缺少时预约会 fail-closed；这不等于生产预约可用。小红书担保支付主动退款、平台售后/退款事件消费、凭证核销和结算仍待协议实现及沙箱、生产验收，未完成前后台退款入口必须明确失败，不能只改本地票权或模拟成功。平台 AI 助手已支持统一平台模型配置、租户月度请求/Token 额度，以及批量票规和新票种创建的多轮任务预览/确认；第一阶段已建立版本化系统知识契约、能力注册表和任务知识冻结，第二阶段已开放订单、票种库存、销售汇总和核销汇总的只读 QueryResult，第三阶段已开放当前租户自有票种的基础信息修改（分销副本除外）预览/确认，阶段五补充租户业务别名、Provider 瞬态失败保护、无密钥 provider-independent 评测回归和 2-5 步低风险复合预览。DeepSeek 新任务默认使用原生工具调用，旧默认配置会在 schema 94 迁移为 `auto`，普通文本不会被当作可执行计划，默认请求超时 120 秒，仍由服务端字段、租户和业务规则校验，模型异常时 fail-closed。复合预览仅编排既有低风险步骤，确认后顺序执行且不承诺跨域原子性；酒店、分销、渠道及所有高风险写入仍未开放给 Agent。团队域已补齐独立合作关系、计划与档案维护、名单/票权归类、并发分批入园、退款投影和双方结算状态机，但这只代表代码与自动化边界收敛，不能替代生产验收。当前生产阻断项仍为真实支付、真实硬件、真实渠道生产验收、部署恢复演练及现场容量/长稳验收。旧库导入不是新系统发布门槛，仅在以后明确提出时作为独立项目处理。当前执行状态以[当前开发推进路线](./current-development-roadmap-2026-08-01.md)和[现场联调准备清单](./field-integration-readiness-checklist.md)为准。
