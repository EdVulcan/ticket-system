...
> **当前审查状态（2026-08-16）**：代码级 P0/P1 已完成一轮闭环并进入自动化，PostgreSQL schema 版本为 95；携程供应商订单沙箱和价格库存推送已完成当前可执行测试，小红书商家自开发小程序已完成登录、商品发布配置、订单创建、担保支付查询收敛，以及酒景套餐预约、撤销和预约状态通知的可重启任务。小红书预约 `/component` 接口已与普通商家 `/mp` 鉴权分离，但服务商 `appid` 和已授权小程序组件凭据尚未配置，缺少时预约会 fail-closed；这不等于生产预约可用。小红书担保支付主动退款、平台售后/退款事件消费、凭证核销和结算仍待协议实现及沙箱、生产验收，未完成前后台退款入口必须明确失败，不能只改本地票权或模拟成功。平台 AI 助手已支持统一平台模型配置、租户月度请求/Token 额度，以及批量票规和新票种创建的多轮任务预览/确认；第一阶段已建立版本化系统知识契约、能力注册表和任务知识冻结，第二阶段已开放订单、票种库存、销售汇总和核销汇总的只读 QueryResult，第三阶段已开放仅针对未上架、未分销票种的基础信息修改预览/确认。DeepSeek 新任务默认使用原生工具调用，旧默认配置会在 schema 94 迁移为 `auto`，普通文本不会被当作可执行计划，默认请求超时 120 秒，仍由服务端字段、租户和业务规则校验，模型异常时 fail-closed。AI 扩展按“系统知识契约与能力注册 → 低风险只读 → 预览型业务操作 → 受控高风险评估”四阶段推进，当前完成第一、二阶段和第三阶段的受控目录范围；订单、库存和报表仅开放明确的只读查询，酒店、分销、渠道及所有高风险写入仍未开放给 Agent。团队域已补齐独立合作关系、计划与档案维护、名单/票权归类、并发分批入园、退款投影和双方结算状态机，但这只代表代码与自动化边界收敛，不能替代生产验收。当前生产阻断项仍为真实支付、真实硬件、真实渠道生产验收、部署恢复演练及现场容量/长稳验收。旧库导入不是新系统发布门槛，仅在以后明确提出时作为独立项目处理。当前执行状态以[当前开发推进路线](./current-development-roadmap-2026-08-01.md)和[现场联调准备清单](./field-integration-readiness-checklist.md)为准。
