...
> **当前审查状态（2026-08-10）**：代码级 P0/P1 已完成一轮闭环并进入自动化，PostgreSQL schema 版本为 77；携程供应商订单沙箱和价格库存推送已完成当前可执行测试，小红书商家自开发小程序已进入登录与票种目录开发阶段。团队域已补齐独立合作关系、计划与档案维护、名单/票权归类、并发分批入园、退款投影和双方结算状态机，但这只代表代码与自动化边界收敛，不能替代生产验收。当前生产阻断项仍为真实支付、真实硬件、真实渠道生产验收、部署恢复演练及现场容量/长稳验收。旧库导入不是新系统发布门槛，仅在以后明确提出时作为独立项目处理。当前执行状态以[当前开发推进路线](./current-development-roadmap-2026-08-01.md)和[现场联调准备清单](./field-integration-readiness-checklist.md)为准。
...
> **实现复核（2026-08-10）**：数据库迁移当前版本为 77。平台治理工作台、供应商 Listing 状态同步、跨供应商组合产品、游客快照归属、POS 收银闭环、按原支付来源分摊的混合退款、供应履约详情、销售订单多供应商责任视图、上下游结算工作台、按票售后、旅行社团队履约闭环以及营业/核销四张正式报表已进入当前实现。团队合作不会从普通分销关系推断，合同产品价也不会创建或覆盖普通 `ProductOffer`；团队计划、业务员/导游/车辆档案、名单票权归类、分批入园、退款投影和结算付款状态均保留审计。小红书游客身份按租户和渠道账号隔离，官方 `session_key` 加密保存且不下发客户端。这些改动改善了业务完整性和运营可见性，但不改变真实支付、渠道、硬件、双方现场制度和生产验收仍是准入门槛的结论。当前实施顺序见[当前开发推进路线](./current-development-roadmap-2026-08-01.md)。
...
当前状态：P0-1 至 P0-6 已通过本地自动化验收，数据库 schema 版本为 77，PostgreSQL 初始化和重复执行幂等测试已通过。`ScenicArea`、租户状态/能力、审计日志、`ProductRevision`、`ProductOffer/SellerListing`、`BundleProduct/BundleVersion/BundleComponent`、`FulfillmentOrder/TicketEntitlement`、整数分账本、结算单、追加式结算调整、售后价差事实、核销冲销审计、团队重复归属保护及小红书渠道游客会话均已进入当前模型。阶段 0 仍未达到生产准入：真实支付商户证书、双方对账验收及现场硬件联调尚未完成；旧库导入仅在以后明确提出时另行验收。
