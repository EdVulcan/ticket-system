...
> **当前审查状态（2026-08-01）**：代码级 P0/P1 已完成一轮闭环并进入自动化，迁移版本为 57；当前生产阻断项已收敛为真实支付、真实硬件、真实渠道协议、真实旧库样本、部署恢复演练及现场容量/长稳验收。当前执行状态以[当前开发推进路线](./current-development-roadmap-2026-08-01.md)和[现场联调准备清单](./field-integration-readiness-checklist.md)为准。
...
- 增加 `ProductRevision`、整数分 `LedgerEntry`、原订单现金/授信构成、`SettlementStatement/Line`，结算按供应履约快照生成，不从当前商品反推；迁移版本已推进到 57，并有迁移幂等回归测试。
...
- 未使用组件可按其零售价分摊独立退票，退款、供应商资金、库存、配额和结算冲减只作用于该组件。已经核销的组件继续拒绝退票。
...
当前状态：P0-1 至 P0-6 已通过本地自动化验收，数据库迁移已推进到版本 57 并通过重复执行幂等测试。`ScenicArea`、租户状态/能力、审计日志、`ProductRevision`、`ProductOffer/SellerListing`、`BundleProduct/BundleVersion/BundleComponent`、`FulfillmentOrder/TicketEntitlement`、整数分账本、结算单、追加式结算调整和售后价差事实均已进入当前模型。阶段 0 仍未达到生产准入：真实旧库完整迁移、真实支付商户证书、双方对账验收及现场硬件联调尚未完成。
...
现有自动化覆盖 SQLite 写入协调、库存并发、外部单号幂等、并发核销、现金支付租户隔离、JWT、OTA 防重放、备份恢复、迁移幂等、支付补偿、管理端四类身份访问边界和 Wails POS 核心交互。它们仍不能替代真实支付商户、渠道协议、硬件和现场业务验收。
