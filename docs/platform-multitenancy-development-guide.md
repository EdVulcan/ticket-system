...
> **闸机协议无关基础架构补充（2026-08-20，维护通道补充 2026-08-21）**：Linux `gate-client` 作为单通道现场运行模块，继续复用云端唯一票权/核销事实和设备 HMAC 认证；本地状态文件为必填，只保存核销请求和物理开闸恢复阶段。硬件驱动通过独立 seam 接入，明确区分 `opened`、`failed` 和 `unknown`，不能把无正文 2xx、超时或 5xx 当作已经开闸；已知失败回报原 `DeviceVerification`，未知结果保持锁定并要求现场确认。旧管理端闸机模拟器不再提交设备原始密钥，真实三辊闸控制板、脉冲、反馈和断电规则仍待现场协议；离线入园尚未启用，必须另行定义签名票包、有效期、重复票冲突和补传审计。新增的维护网关默认关闭，仅由租户管理员按原因创建短时 WSS 会话；gate-client 只把 SSH 转发到闸机本机 `127.0.0.1:22`，使用独立维护凭据和 `onsite.maintenance` 权限，不接受任意目标地址，不改变票务或开闸事实。
