...
> **当前审查状态（2026-07-31）**：最新全面复核重新打开了阶段 0。本文中的目标规则继续有效，但既有“已落地/已解决”状态不得单独作为完成依据；开发前还必须阅读[当前阶段目标一致性全面审查报告](./current-stage-goal-alignment-audit-2026-07-31.md)，并以其中 P0 阻断项和验收矩阵作为当前实现状态。

- 文档状态：后续开发的架构与验收基线
- 审计日期：2026-07-31
- 审计分支：`codex/embedded-sqlite-hardening`
- 审计基线：`ed0e794`；当前修复与文档变更仍在工作区，尚未提交
...
> **实现复核（2026-08-01）**：数据库迁移当前版本为 43。平台治理工作台已进入主线，供应商 Listing 状态同步已加入主线；这些改动改善了运营可见性，但不改变真实支付、渠道、硬件、财务制度和现场验收仍是生产准入门槛的结论。
...
| 旅行社团队业务 | 30% | 合同、业务员、导游、车辆、团队、名单和分批入园基础已落地；团队价格/授信/发票仍待现场确认 |
...
- Go 模块化单体、Vue 管理端、Electron 窗口端。
- SQLite WAL、串行写协调器、自动备份和单服务部署。
- 租户、后台用户、员工及基础角色。
...
- 微信 V3、支付宝回调入口完成租户作用域、签名/解密、金额校验和重复通知幂等；SQLite 持久化支付查单任务，进程重启后可补建并退避重试。
...
- 增加 `ProductRevision`、整数分 `LedgerEntry`、原订单现金/授信构成、`SettlementStatement/Line`，结算按供应履约快照生成，不从当前商品反推；迁移版本已推进到 19，并有迁移幂等回归测试。
...
- 不得为持久化任务和回调重试提前引入 Redis 或消息队列；先使用 SQLite 任务表和 outbox。
...
- 未支付超时、主动取消和现金按票退款已存在；改期、换票、补票、作废和异常订单工作台仍缺失。
...
- POS API 地址支持 `VITE_API_URL` 配置；Electron 已恢复 `sandbox` 和 `webSecurity`，但硬件桥接仍需最小权限审计。
- 管理端和 POS 已去掉相关 `dangerouslyUseHTMLString` 用法；前端自动化测试仍待建立。
- 管理端和桌面端没有单元测试、组件测试或 E2E 测试。
...
现阶段继续使用共享 SQLite 和逻辑租户隔离是合理的：

- 不需要为每个景区单独创建数据库。
- 每租户独立备份/恢复不是当前默认能力；如未来需要，应先设计租户导出、校验和恢复流程。
- SQLite 没有 PostgreSQL Row Level Security，因此应用作用域、数据库约束和负向测试必须更加严格。
- 单服务和 SQLite 适合当前低运维目标；只有在实测写入峰值、多实例容灾或跨园区高可用确有需要时再迁移 PostgreSQL。
...
- 内部统一命令处理预下单、确认、取消、退款、核销和凭证；携程、美团、智游宝等只作为适配器。
- 请求签名应覆盖方法、路径、时间、nonce 和正文摘要，采用 HMAC-SHA256 等成熟方案。
- 使用 SQLite 持久化任务表和 outbox 即可实现回调重试、查单补偿和人工重放，不需要为了这些能力立即增加 Redis 或消息队列。
...
这些模块可以共享同一 Go 程序和同一 SQLite 数据库，但不应越过服务接口直接修改其他模块的数据。
...
当前状态：P0-1 至 P0-6 已通过本地自动化验收，数据库迁移已推进到版本 43 并通过重复执行幂等测试。`ScenicArea`、租户状态/能力、审计日志、`ProductRevision`、`ProductOffer/SellerListing`、`FulfillmentOrder/TicketEntitlement`、整数分账本和结算单均已进入当前模型。阶段 0 仍未达到生产准入：旧数据完整迁移、数据库复合约束、真实支付商户证书、财务对账、前端 E2E 及现场硬件联调尚未完成。

### 阶段 1：完成景区直营生产闭环

状态：后端已具备库存预占/释放、票权益快照、现金退款、支付补偿任务、POS 班次、打印任务和设备告警基础；改期/作废/补打、数字退款供应商适配、报表口径确认、真实设备协议和现场全链路验收仍未完成。

- 不可变票权益快照和可靠库存预占。
- 未支付超时、主动取消和异常订单工作台。
- 微信/支付宝通知、主动查单、数字退款和日终对账。
- 现金部分退款已支持；改期、作废、补打和完整售后时间线仍待实现。
...
状态：报价、销售映射、供应履约单、票权益、资金账本和结算单基础已落地，并有跨景区核销和篡改防护测试；供应商履约工作台、合作申请/审批 UI、价格与库存同步、授信冻结转换、退款责任和双方对账验收仍未完成。
...
状态：独立渠道账号、密钥轮换、商品映射、正文签名、请求正文冲突检测及基础内部路由已落地；智游宝/携程/美团等真实适配器、渠道预下单锁库、限流/IP 白名单、账单导入、多方对账和数字退款适配仍待完成。

- 独立渠道账号、密钥轮换、商品映射、权限和停用。
- 预下单、确认、查单补偿、凭证、取消、退款和可靠回调。
- 接口日志、标准错误码、限流、沙箱和联调工具。
- 渠道账单导入和订单、支付、退款、核销多方对账。
- 智游宝、携程、美团等通过适配器接入统一内部模型。

### 阶段 4：旅行社团队业务

状态：合同、业务员、导游、车辆、团队、名单和分批入园模型/API 已落地；团队价格、预付款/授信、发票、团队确认单、团队售后和正式结算流程仍需现场制度确认及实现。

- 旅行社、业务员、导游、车辆和合同档案。
- 团队计划、名单导入、人数变更、分批入园和团队售后。
- 预付款、授信、挂账、月结、返佣和应收账龄。
- 团队确认单、结算单、发票和对账确认。
...
现有后端测试覆盖 SQLite 写入协调、库存并发、外部单号幂等、并发核销、现金支付租户隔离、JWT、OTA 防重放、备份、迁移幂等和支付补偿。它们仍不能替代真实多租户供应链、支付商户、硬件和前端 E2E 验收。
...
- 平台、供应商、分销商、旅行社分别执行核心工作流 E2E。
- POS 使用真实打印机、扫码器和证件阅读器验收，不能以 mock 返回作为通过。
- 支付窗口被关闭、断网、终端重启后订单可恢复和继续处理。
- 设备断线后在规定时间内显示离线并产生告警。
- 所有退款、授信调整、人工核销和平台代管动作可在审计日志中还原。

当前自动化覆盖已增加：跨景区核销拒绝、景区 API 租户边界、租户冻结/能力独立、平台作用域、支付通知幂等与补偿任务、现金按票退款、商品软删除后的预占释放、分销来源/结算价防篡改、售票权益快照、渠道请求正文冲突和数据库迁移幂等。真实支付商户联调、渠道账单对账、POS 硬件和前端 E2E 仍不在本地自动化范围内。
...
- `desktop/src/renderer/src/views/SalesView.vue`：窗口售票、核销和交班原型。
- `desktop/src/main/index.ts`：打印和读卡 mock。

## 12. 已确认决策

- 产品目标是建设更适合本景区的系统，不复刻智游宝菜单。
- 智游宝对接文档只作为外部渠道适配参考，不定义系统全部能力。
- 系统商家是平台最高治理者；景区供应商、分销商和旅行社是租户。
- 不同景区租户的数据必须独立，A 景区门票绝不能在 B 景区核销。
- 分销商可销售供应商产品，但票权益和核销责任仍属于对应景区。
- 现阶段保留模块化单体、单服务和 SQLite，避免引入 Redis、MySQL 或微服务。
...
2. 分销结算是在下单、游客支付、出票、核销还是账期结束时确认。
3. 分销商是否允许一个销售订单包含多个供应商商品。
4. 合作暂停后，未支付订单和已支付存量票分别如何处理。
5. 实际使用的 OTA、智游宝角色、商品编码和账单格式。
