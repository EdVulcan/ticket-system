...
状态：独立渠道账号、密钥轮换、商品映射、正文签名、请求正文冲突检测、请求失败授权重试、渠道订单综合查询、账单导入及逐笔差异下钻已落地；携程供应商预下单、支付同步确认、预下单取消、未核销整单取消、订单查询和可靠核销通知已完成，订单遍历 18 项已全部通过，核销通知获得携程真实沙箱 `0000` 成功响应。部分退订和仅退款仍按合同范围确认，生产接入仍需生产凭据和网络来源配置。美团仍待协议与凭据。智游宝上游供应接入不在当前阶段。
