...
> **当前审查状态（2026-08-14）**：代码级 P0/P1 已完成一轮闭环并进入自动化，PostgreSQL schema 版本为 88；携程供应商订单沙箱和价格库存推送已完成当前可执行测试，小红书商家自开发小程序已完成登录、商品发布配置、订单创建、担保支付查询收敛，以及酒景套餐预约、撤销和预约状态通知的可重启任务。小红书预约 `/component` 接口已与普通商家 `/mp` 鉴权分离，但服务商 `appid` 和已授权小程序组件凭据尚未配置，缺少时预约会 fail-closed；这不等于生产预约可用。小红书担保支付主动退款、平台售后/退款事件消费、凭证核销和结算仍待协议实现及沙箱、生产验收，未完成前后台退款入口必须明确失败，不能只改本地票权或模拟成功。平台 AI 助手已支持统一平台模型配置、租户月度请求/Token 额度、受限票规计划预览和确认审计；它不会直接修改业务数据，未配置、额度耗尽或模型输出不符合 DSL 时 fail-closed。团队域已补齐独立合作关系、计划与档案维护、名单/票权归类、并发分批入园、退款投影和双方结算状态机，但这只代表代码与自动化边界收敛，不能替代生产验收。当前生产阻断项仍为真实支付、真实硬件、真实渠道生产验收、部署恢复演练及现场容量/长稳验收。旧库导入不是新系统发布门槛，仅在以后明确提出时作为独立项目处理。当前执行状态以[当前开发推进路线](./current-development-roadmap-2026-08-01.md)和[现场联调准备清单](./field-integration-readiness-checklist.md)为准。
...
- 管理端悬浮 AI 助手只负责把租户当前输入映射为同一组受限 DSL，然后调用上述预览流程；模型不能直接写商品、票规、授权或库存，也不能提供跨租户 ID。模型请求只携带当前租户候选票种/检票点和用户描述，服务端仍会再次解析名称、校验租户与景区归属、生成 revision 哈希并要求确认。
...
当前状态：P0-1 至 P0-6 已通过本地自动化验收，数据库 schema 版本为 88，PostgreSQL 初始化和重复执行幂等测试已通过。`ScenicArea`、租户状态/能力、供应商业态、审计日志、`ProductRevision`、`ProductOffer/SellerListing`、`BundleProduct/BundleVersion/BundleComponent`、`FulfillmentOrder/TicketEntitlement`、整数分账本、结算单、追加式结算调整、售后价差事实、核销冲销审计、团队重复归属保护、小红书渠道游客、商品、订单、券码、酒景预约权益及持久预约任务、平台 AI 配置和租户月度用量账本均已进入当前模型。阶段 0 仍未达到生产准入：真实支付商户证书、小红书服务商组件授权、资金退款与售后事件消费、双方对账验收及现场硬件联调尚未完成；旧库导入仅在以后明确提出时另行验收。
