...
当前状态：P0-1 至 P0-6 已通过本地自动化验收，数据库迁移已推进到版本 56 并通过重复执行幂等测试。`ScenicArea`、租户状态/能力、审计日志、`ProductRevision`、`ProductOffer/SellerListing`、`FulfillmentOrder/TicketEntitlement`、整数分账本、结算单、追加式结算调整和售后价差事实均已进入当前模型。阶段 0 仍未达到生产准入：真实旧库完整迁移、真实支付商户证书、双方对账验收及现场硬件联调尚未完成。
