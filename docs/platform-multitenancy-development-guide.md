...
> **小红书商家自研链路补充（2026-09-03）**：小程序解绑服务商后，普通 `/mp` 商品、订单、支付查询和普通券核销继续使用租户自有商家凭据；预售券预约仍严格依赖服务商 `/component` 凭据。普通券核销已通过持久协调记录衔接外部 `verify_id` 与本地票务事实：外部结果未知时不开闸且不自动重发，prepared/外部确认/本地待提交阶段可由 worker 在进程重启后恢复；schema 现为 108，并增加券核销协调表、票权占用、幂等索引和跨租户归属触发器。自动化已覆盖一次性外部调用、未知结果阻断、prepared 恢复及数据库归属保护；真实沙盒券核销、退款/售后消费、结算和生产凭据仍未验收，不得标记为生产可用。
