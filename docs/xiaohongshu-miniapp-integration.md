...
- PostgreSQL schema 83 增加酒景套餐售后预约权益。每份套餐独立预约，预约成功前不占用具体日期的票库存和房晚，也不向游客展示可用票码；后台可查看待预约、已预约、已退款和已过期权益。
...
- 先售券后预约页面、服务端库存事务、小红书预约/状态同步客户端及失败重试已经完成代码和自动化。真实沙箱仍需依次验证预售券商品同步、`0.01` 元担保支付、预约、取消预约、改约、退款和过期；生产环境仍需重新验收。凭证核销、售后退款、结算请求和消息事件业务分发不得在未联调时显示为可用或模拟成功。
