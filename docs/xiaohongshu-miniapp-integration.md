...
### 2026-09-06 正式小程序当前阻塞状态

- 当前租户 `SYS001` 使用正式小程序 AppID `672997dc4889610001aa80b2`，渠道账号本身已启用并标记为生产环境；商家凭据连接测试通过，POI 查询也返回 1 个可用门店。
- 当前仍沿用本地联调门票映射：产品 `5`“小红书沙盒联调测试票（禁止现场使用）”、外部商品编码 `XHS_SANDBOX_001`、发布 SKU `XHS_SANDBOX_001_SKU`。这组配置只能用于继续排查配置，不代表正式商品已经可售。
- 正式环境类目查询实际返回 `data: null`，诊断结果为“已读取 0 个类目，其中 0 个支持交易”，因此连接诊断的 `ready=false`。现有发布配置使用的类目 ID `65f997513e946300016b32de` 被小红书以错误 `420209` 拒绝，原因是“类目非生活服务类目，请确认参数是否正确”。
- 该阻塞发生在小红书正式小程序的交易类目/资质侧，不是本地 AppID、凭据或 POI 读取失败。没有可用的生活服务末级交易类目前，不应继续保存并同步商品，也不能把商品同步、下单、支付、券核销或结算标记为正式可用。
- 小红书工作暂缓，后续恢复顺序应为：先在正式小程序后台取得可交易生活服务类目并重新连接测试，再配置正确类目和 POI，最后按本文档的正式商品、订单、支付、核销、售后和结算验收顺序执行。此前的沙盒/正式混用配置不作为通过依据。
...
商品审核回调只有明确的文字状态（例如 `approved`、`rejected`、`offline`）才会改变本地审核闸门。未从小红书正式 webhook 样本确认的数字状态不会被猜测映射，统一保留为待审核并进入人工复核，避免错误放行商品。
