...
生产数据库已切换到 PostgreSQL，当前 schema 版本为 80。Go 服务、管理端静态资源和业务任务仍由一个应用进程承载，不引入 Redis、MySQL、消息队列或微服务。

应用启动时必须拒绝高于自身支持版本的数据库 schema，避免旧程序在回滚后忽略新增的租户或业态授权边界。schema 80 首次上线属于前向迁移：部署前必须完成备份，不能直接回滚到不认识 `supplier_business_types` 的旧二进制。
