...
func (s *DeviceProvisioningService) CreateLease(req ProvisioningLeaseRequest) (*ProvisioningLeaseResult, error) {
	...
	err := model.Write(func(tx *gorm.DB) error {
		...
		// the periodic cleanup worker has not run yet. Clearing the envelope here
		// also bounds how long an abandoned claim can retain device credentials.
		if err := tx.Model(&model.DeviceProvisioningLease{}).
			...
			Updates(map[string]interface{}{"status": "expired", "encrypted_bundle": "", "installer_public_key": ""}).Error; err != nil {
			return err
		}
		var existing int64
		if err := tx.Model(&model.DeviceProvisioningLease{}).
...
// RevokeLease invalidates a pending/claimed lease without touching the
// device's current access key. It is useful when a binding code was copied to
// the wrong workstation or the field installation is cancelled.
...
func (s *DeviceProvisioningService) ExpireLeases(now time.Time, limit int) (int, error) {
	...
	for _, row := range rows {
		if err := model.Write(func(tx *gorm.DB) error {
			result := tx.Model(&model.DeviceProvisioningLease{}).Where("id = ? AND status IN ? AND expires_at <= ?", row.ID, []string{"pending", "claimed"}, now).Updates(map[string]interface{}{"status": "expired", "encrypted_bundle": "", "installer_public_key": ""})
			return result.Error
		}); err != nil {
			return count, err
		}
