# Scenic Ticketing Runtime Skill: Batch Unpublished Product Updates
...
checkpoint/rule changes.

## Target and scope

The request must provide at least two exact current-tenant product names. The
server resolves every name and requires every target to remain offline,
non-distributable, and backed by the expected current ProductRevision.
...
order, rechecks tenant ownership, offline/non-distributable status and version,
then commits every product revision in one transaction. Any conflict rejects
the whole batch; no partial success is reported.
