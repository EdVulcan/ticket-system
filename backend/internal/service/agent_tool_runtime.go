...
<domain_skill>` + domainSkill + `</domain_skill>
	...
	systemPrompt += `
	对于修改票种基础信息的请求，必须直接调用 prepare_ticket_product_update；用 target_scope 表达用户给出的目标范围，product_name 只为旧任务兼容，字段只填写用户明确提供的值。服务端允许修改当前租户自有票种，包括已上架或允许分销的自有票种；分销副本仍会拒绝，并在确认时再次锁定当前版本。“未核销随时退/随时退”应填写 refund_type=free，这是产品政策更新，不是订单退款。不要通过该工具修改检票点、规则组、上架状态、分销授权、渠道、库存预占或资金事实。`
	systemPrompt += `
