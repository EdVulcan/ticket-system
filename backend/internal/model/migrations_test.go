...
func TestMigrationsReachCurrentVersionAndAreIdempotent(t *testing.T) {
	...
	if latest.Version != 23 {
		t.Fatalf("latest migration=%d, want 23", latest.Version)
	}
	for _, table := range []string{"product_revisions", "ledger_entries", "channel_accounts", "tour_groups", "pos_shifts", "settlement_statements"} {
		var count int64
