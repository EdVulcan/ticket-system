...
type XiaohongshuBookingOperation struct {
	...
	Type                     string     `gorm:"size:20;not null;index;check:chk_xiaohongshu_booking_operations_type,type IN ('book','revoke','refund')" json:"type"`
	...
	CompletedAt              *time.Time `gorm:"check:chk_xiaohongshu_booking_operations_semantics,(((type = 'book' AND external_book_order_id <> '') OR (type IN ('revoke','refund') AND external_book_order_id <> '' AND platform_book_id <> '')) AND (type = 'book' OR status IN ('pending','remote_succeeded','completed','failed')) AND (status NOT IN ('remote_succeeded','confirm_pending','compensation_pending','completed') OR platform_book_id <> '') AND (failed_from_stage = '' OR failed_from_stage IN ('pending','remote_succeeded','confirm_pending','compensation_pending')) AND (failed_from_stage = '' OR status = 'failed') AND ((status IN ('completed','failed')) = (completed_at IS NOT NULL)) AND ((status IN ('pending','remote_succeeded','confirm_pending','compensation_pending')) = (next_attempt_at IS NOT NULL)))" json:"completed_at,omitempty"`
}
