...
const (
	...
	// Resolving an unknown external voucher result can either admit a visitor
	// or release a ticket lock, so it is intentionally admin-only. It is kept
	// separate from ordinary channel write access to prevent a product operator
	// or channel configurator from manufacturing an admission fact.
	PermissionXiaohongshuVoucherResolve = "channels.xiaohongshu.voucher.resolve"
	// Xiaohongshu after-sale callbacks can pause fulfillment, so their manual
	// resolution is a separate administrator-only capability. Read access is
	// kept distinct from mutation even though both are hidden from delegated
	// role defaults until the workflow is proven in production.
	PermissionXiaohongshuRefundRead    = "channels.xiaohongshu.refund.read"
	PermissionXiaohongshuRefundResolve = "channels.xiaohongshu.refund.resolve"
